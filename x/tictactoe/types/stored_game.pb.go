...
type StoredGame struct {
	...
	XPlayer  string `protobuf:"bytes,4,opt,name=xPlayer,proto3" json:"xPlayer,omitempty"`
	YPlayer  string `protobuf:"bytes,5,opt,name=yPlayer,proto3" json:"yPlayer,omitempty"`
}
...
func (m *StoredGame) GetNextTurn() string {
	if m != nil {
		return m.NextTurn
	}
	return ""
...
func (m *StoredGame) GetXPlayer() string {
	if m != nil {
		return m.XPlayer
	}
	return ""
...
func (m *StoredGame) GetYPlayer() string {
	if m != nil {
		return m.YPlayer
	}
	return ""
...
func init() {
	proto.RegisterType((*StoredGame)(nil), "alice.checkers.tictactoe.StoredGame")
}
...
var fileDescriptor_890ea4c29805d5e1 = []byte{
	...
	0xce, 0x48, 0x4d, 0xce, 0x4e, 0x2d, 0x2a, 0xd6, 0x83, 0xab, 0x55, 0xea, 0x60, 0xe4, 0xe2, 0x0a,
	0x06, 0xab, 0x77, 0x4f, 0xcc, 0x4d, 0x15, 0x12, 0xe1, 0x62, 0xcd, 0xcc, 0x4b, 0x49, 0xad, 0x90,
	0x60, 0x54, 0x60, 0xd4, 0xe0, 0x0c, 0x82, 0x70, 0x40, 0xa2, 0x49, 0xf9, 0x89, 0x45, 0x29, 0x12,
	0x4c, 0x10, 0x51, 0x30, 0x47, 0x48, 0x8a, 0x8b, 0x23, 0x2f, 0xb5, 0xa2, 0x24, 0xa4, 0xb4, 0x28,
	0x4f, 0x82, 0x19, 0x2c, 0x01, 0xe7, 0x0b, 0x49, 0x70, 0xb1, 0x57, 0x04, 0xe4, 0x24, 0x56, 0xa6,
	0x16, 0x49, 0xb0, 0x80, 0xa5, 0x60, 0x5c, 0x90, 0x4c, 0x25, 0x54, 0x86, 0x15, 0x22, 0x03, 0xe5,
	...
	0x49, 0x46, 0x69, 0x92, 0x5e, 0x72, 0x7e, 0xae, 0x3e, 0xd8, 0x27, 0xfa, 0x30, 0x9f, 0xe8, 0x57,
	0xe8, 0x23, 0xfc, 0x5d, 0x52, 0x59, 0x90, 0x5a, 0x9c, 0xc4, 0x06, 0xf6, 0xb2, 0x31, 0x20, 0x00,
	0x00, 0xff, 0xff, 0x14, 0xae, 0xc5, 0xc2, 0x11, 0x01, 0x00, 0x00,
}
...
func (m *StoredGame) MarshalToSizedBuffer(dAtA []byte) (int, error) {
	...
	if len(m.YPlayer) > 0 {
		i -= len(m.YPlayer)
		copy(dAtA[i:], m.YPlayer)
		i = encodeVarintStoredGame(dAtA, i, uint64(len(m.YPlayer)))
		i--
		dAtA[i] = 0x2a
	}
	if len(m.XPlayer) > 0 {
		...
		i = encodeVarintStoredGame(dAtA, i, uint64(len(m.XPlayer)))
		i--
		dAtA[i] = 0x22
	}
	if len(m.NextTurn) > 0 {
		...
		i = encodeVarintStoredGame(dAtA, i, uint64(len(m.NextTurn)))
		i--
		dAtA[i] = 0x1a
	}
	if len(m.Board) > 0 {
...
func (m *StoredGame) Unmarshal(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 5:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field YPlayer", wireType)
			}
			var stringLen uint64
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			m.YPlayer = string(dAtA[iNdEx:postIndex])
			iNdEx = postIndex
		default:
			iNdEx = preIndex
