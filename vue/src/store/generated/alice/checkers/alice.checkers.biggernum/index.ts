...
import { Games } from "./module/types/biggernum/games"
import { Params } from "./module/types/biggernum/params"
import { SystemInfo } from "./module/types/biggernum/system_info"


export { Games, Params, SystemInfo };
...
const getDefaultState = () => {
	return {
				...
				_Structure: {
						Games: getStructure(Games.fromPartial({})),
...
export default {
	...
	getters: {
			...
			return state.Games[JSON.stringify(params)] ?? {}
				...
				getGamesAll: (state) => (params = { params: {}}) => {
					if (!(<any> params).query) {
						(<any> params).query=null
					}
			return state.GamesAll[JSON.stringify(params)] ?? {}
		},
	...
	actions: {
		init({ dispatch, rootGetters }) {
			console.log('Vuex module: alice.checkers.biggernum initialized!')
		...
		async QueryGames({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				const key = params ?? {};
				const queryClient=await initQueryClient(rootGetters)
				let value= (await queryClient.queryGames( key.index)).data
		...
		async QueryGamesAll({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				const key = params ?? {};
				const queryClient=await initQueryClient(rootGetters)
				let value= (await queryClient.queryGamesAll(query)).data
				
					
				while (all && (<any> value).pagination && (<any> value).pagination.next_key!=null) {
					let next_values=(await queryClient.queryGamesAll({...query, 'pagination.key':(<any> value).pagination.next_key})).data
					value = mergeResults(value, next_values);
				}
				commit('QUERY', { query: 'GamesAll', key: { params: {...key}, query}, value })
				if (subscribe) commit('SUBSCRIBE', { action: 'QueryGamesAll', payload: { options: { all }, params: {...key},query }})
				return getters['getGamesAll']( { params: {...key}, query}) ?? {}
			} catch (e) {
				throw new Error('QueryClient:QueryGamesAll API Node Unavailable. Could not perform query: ' + e.message)
